 ...
 * When the tick is called, it closes the circuit, making the short for one clock cycle. This means that when the tick method is called, it will let one single burst of data (one instance of the object that represents data (class InfoSecData)) flow through its source edge to the destination edge.
 ...
 * This node will not be run on a thread, but it will be a class instance... yes I know I'm creating a new class instance for a simple short circuit, but lets not sacrifice efficiency over good abstraction.
 */
public class ShortCircuitNodeTask extends TaskGraphNode
{
    ...
    {
        InfoSecData data = sources.get(0).receiveData();
